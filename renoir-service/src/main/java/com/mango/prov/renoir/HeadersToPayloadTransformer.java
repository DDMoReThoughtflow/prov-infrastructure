...
public class HeadersToPayloadTransformer {
	...
	public Object transform(Message<?> message) {
		...
		if(!StringUtils.isEmpty(originalPayloadKey.trim())) {
			newPayload.put(originalPayloadKey.trim(), getPayload(message));
		...
		Parcel parcel = getParcel(message);
		if(parcel != null) {
			parcel.setPayload(newPayload);
			return parcel;
		}
		return newPayload;
	}

	private Parcel getParcel(Message<?> message) {
		Object parcel = message.getPayload();
